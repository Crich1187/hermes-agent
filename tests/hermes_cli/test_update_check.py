...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    """When cache is fresh for the active repo, check_for_updates avoids git."""
    ...
    cache_file.write_text(
        json.dumps({"ts": time.time(), "behind": 3, "repo": str(repo_dir.resolve())})
    )
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    cache_file.write_text(json.dumps({"ts": 0, "behind": 1, "repo": str(repo_dir.resolve())}))
...
def test_check_for_updates_prefers_active_package_repo(tmp_path, monkeypatch):
    """Editable installs should use the imported package checkout, not HERMES_HOME/hermes-agent."""
    import hermes_cli.banner as banner
    ...
    called_cwds = {call.kwargs.get("cwd") for call in mock_run.call_args_list}
    assert str(project_root) in called_cwds


def test_check_for_updates_ignores_cache_from_different_repo(tmp_path, monkeypatch):
    """Behind-count caches from another checkout must not be reused."""
    from hermes_cli.banner import check_for_updates
    ...
    cache_file = tmp_path / ".update_check"
    cache_file.write_text(
        json.dumps({"ts": time.time(), "behind": 1232, "repo": str((tmp_path / 'other-repo').resolve())})
    )
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("hermes_cli.banner._resolve_repo_dir", return_value=repo_dir), \
         patch("hermes_cli.banner.subprocess.run", side_effect=[fetch_result, rev_list_result]) as mock_run:
        result = check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    default_home.mkdir()
    (default_home / ".update_check").write_text('{"ts":1,"behind":50,"repo":"/tmp/repo"}')

    profiles_root = default_home / "profiles"
    for name in ("ops", "dev"):
        p = profiles_root / name
        p.mkdir(parents=True)
        (p / ".update_check").write_text('{"ts":1,"behind":50,"repo":"/tmp/repo"}')
...
def test_invalidate_update_cache_no_profiles_dir(tmp_path):
    ...
    default_home = tmp_path / ".hermes"
    default_home.mkdir()
    (default_home / ".update_check").write_text('{"ts":1,"behind":5,"repo":"/tmp/repo"}')
